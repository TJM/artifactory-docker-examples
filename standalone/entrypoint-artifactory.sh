...
testDirectoryPermissions () {
    ...
    logger "Testing directory (${dir_to_check}) has read/write permissions"
    ...
    if echo test > ${test_file} 1> /dev/null 2>&1; then
        ...
        if cat ${test_file} > /dev/null; then
            logger "${dir_to_check} has read/write permissions for ${ARTIFACTORY_USER_NAME}"
            rm -f ${test_file}
        else
            error=true
            error_type=readable
    ...
    if [ "${error}" == true ]; then
        ...
        logger "###########################################################"
        logger "${dir_to_check} is NOT ${error_type}!"
        logger "${stat_data}"
        logger "Mounted directory must be ${error_type} by user '${ARTIFACTORY_USER_NAME}' (id ${ARTIFACTORY_USER_ID})"
        logger "###########################################################"
        errorExit "Directory ${dir_to_check} has bad permissions for user '${ARTIFACTORY_USER_NAME}'"
    fi
}
...
# Test permissions on ARTIFACTORY_HOME and ARTIFACTORY_DATA
testPermissions () {
    # ARTIFACTORY_HOME folder
    testDirectoryPermissions ${ARTIFACTORY_HOME}
